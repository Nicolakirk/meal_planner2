...
import { Modal, Dropdown,Card, Button, Form, Row, Col, CardBody, CardTitle, CardText, CardLink, CardImg } from 'react-bootstrap'
...
const RecipeCard = ({item, recipeArray, setRecipeArray})=>{
    ...
    const handleOKClick = (day) => {
        ...
        if (selectedDay && !selectedDays.includes(selectedDay)) {
            planner.addOneToPlanner(item.recipe.label, item.recipe.ingredientLines, item.recipe.url, selectedDay);
            setSelectedDays([...selectedDays, selectedDay]);
            setSelectedDay(null);
            setShowModal(false);
        } else if (selectedDays.includes(selectedDay)) {
            setErrorMessage(`You have already selected ${selectedDay}. Choose another day.`);
          } else {
    ...
    return(
            ...
            <Card >
                <CardBody align="center" >
                    ...
                    <img src={item.recipe.image} alt={item.recipe.label}  className="img-thumbnail" />
<Card.Title> <h4 >{item.recipe.label}</h4></Card.Title>
<Card.Text> Dishtype: {item.recipe.dishType[0]}</Card.Text>
<Card.Text> <p> Time: {item.recipe.totalTime} minutes</p></Card.Text>
<Card.Text>  <h4> Ingredients</h4> </Card.Text>
<Card.Text>  {item.recipe.ingredientLines.map((ingredient) => ( 
 <div key={ingredient}>{ingredient}</div> 
           ))}</Card.Text>
 <Card.Link href={item.recipe.url} target="_blank" rel="noopener noreferrer"
      > View Recipe </Card.Link>
                </CardBody>
